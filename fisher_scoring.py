...
def exchange(df,L,theta):
    ...
    while l <= L:
        if old:
            i = random.choice([j for j in range(1,d+1)]) #変更点
            tmp = random.sample([j for j in range(1,n+1)],2) #変更点
        if new:
            tmp = random.sample([j for j in range(2,n+1)],2) #変更点

        s,t = min(tmp),max(tmp)
        #si成分とti成分を入れ替える
        Z_proposal = Z.copy()
        if old:
            Z_proposal[s-1][i-1],Z_proposal[t-1][i-1] = Z_proposal[t-1][i-1],Z_proposal[s-1][i-1] #変更点
        if new:
        ...
        if old:
            df_aux_tmp[s-1][i-1] = df_aux[t-1][i-1] #変更点 
            df_aux_tmp[t-1][i-1] = df_aux[s-1][i-1] #変更点
        if new:
