...
if __name__ == "__main__":
    ...
    unit, condition, repeat = 0, 0, 0 #unit: 0~32, conditionは0と1だとrepeatは124,126まで埋まっているが, 2以降はrepeatは1しか埋まっていない.
    univariate_spike_times = events[unit, condition, repeat] #(number, 1)
    ...
    # unitwise_plot(events, condition)

    ###カウントデータに変換
    # merged = []
    # for i in range(33):
    #     tmp = events[i, condition, repeat].flatten().tolist()
    #     merged = merged + tmp
    # merged.sort()
